 ...
 * Fixed version v3 that addresses the 406 Not Acceptable error when querying expert_profiles
 */
class ExpertService {
  /**
  ...
  async getExpertProfile(userId: string): Promise<ExpertProfile | null> {
    try {
      console.log('Getting expert profile for user ID:', userId);
  ...
  async deleteExpertProfile(userId: string): Promise<boolean> {
    try {
      ...
      if (!profile) {
        console.log('No profile found to delete for user ID:', userId);
        return false;
      }
  ...
  async getTopExperts(limit: number = 10): Promise<any[]> {
    try {
      ...
      // MODIFIED: Use a more basic query to avoid potential issues
      ...
      // Get user information for each expert profile
      const userIds = expertProfiles.map(profile => profile.user_id);
      ...
      // Combine the expert profiles with endorsement counts and user data
      const expertsWithCounts = expertProfiles.map(profile => {
        ...
        return {
          ...
          user_name: user.email ? user.email.split('@')[0] : 'Expert',
          // Add placeholder avatar URL
          user_avatar_url: null
        };
      });
      
      // Sort by endorsement count (descending) and limit the results
