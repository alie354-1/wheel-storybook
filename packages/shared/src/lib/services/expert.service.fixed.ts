    ...
    try {
      ...
      const expertsWithCounts = expertProfiles.map(profile => {
        return {
          ...
          user_name: profile.user?.user_metadata?.first_name 
            ? `${profile.user.user_metadata.first_name} ${profile.user.user_metadata.last_name || ''}`
            : 'Expert',
          // Add avatar URL for convenience
          user_avatar_url: profile.user?.user_metadata?.avatar_url || null
        };
      });
